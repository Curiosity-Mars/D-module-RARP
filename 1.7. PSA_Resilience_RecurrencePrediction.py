# PSA_Resilience_RecurrencePrediction.py
...
def compute_dmodule_features(df, psa_cols):
    ...
    df['t_star'] = a_t.apply(compute_t_star, axis=1)
    return df
...
def evaluate_prediction(df):
    ...
    print("(Based on assumption: t* exists → recurrence)")
    ...
    plt.title("ROC Curve: t* predicting recurrence")
    plt.grid(True)
    plt.show()
